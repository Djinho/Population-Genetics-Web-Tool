...
CREATE INDEX IF NOT EXISTS idx_chrom ON chromosome_data (CHROM);
CREATE INDEX IF NOT EXISTS idx_pos ON chromosome_data (POS);
CREATE INDEX IF NOT EXISTS idx_gene_refgene ON chromosome_data (Gene_refGene);
''')

# Prepare the INSERT statement
insert_stmt = '''
INSERT INTO chromosome_data (ID, CHROM, POS, REF, ALT, QUAL, FILTER, FORMAT, ANNOVAR_DATE, Func_refGene, Gene_refGene, ACB, ASW, BEB, CDX, CEU, CHB, CHS, CLM, ESN, FIN, GBR, GIH, GWD, IBS, ITU, JPT, KHV, LWK, MSL, MXL, PEL, PJL, PUR, SIB, STU, TSI, YRI, AFR, AMR, EAS, EUR, SAS)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
'''
...
with open('gene_schema_insert.csv', 'r') as csvfile:
    ...
    print(reader.fieldnames)  # Print column names
    for row in reader:
        ...
        if row['CHROM'] == 'chr1':
            cursor.execute(insert_stmt, (
                ...
                row.get('Func_refGene', ''),  # Use get method to avoid KeyError
                row.get('Gene_refGene', ''),  # Use get method to avoid KeyError
