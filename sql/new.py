...
INSERT INTO chromosome_data (
    ID, CHROM, POS, REF, ALT, QUAL, FILTER, FORMAT, ANNOVAR_DATE, Func_refGene, Gene_refGene,
    ACB, ASW, BEB, CDX, CEU, CHB, CHS, CLM, ESN, FIN, GBR, GIH, GWD, IBS, ITU, JPT, KHV, LWK,
    MSL, MXL, PEL, PJL, PUR, SIB, STU, TSI, YRI, AFR, AMR, EAS, EUR, SAS
)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
'''
    ...
    for row in reader:
        ...
        if row['CHROM'] == 'chr1':
            cursor.execute(insert_stmt, (
                row['ID'], row['CHROM'], int(row['POS']), row['REF'], row['ALT'],
                row['QUAL'], row['FILTER'], row['FORMAT'],
                datetime.strptime(row['ANNOVAR_DATE'], '%Y-%m-%d').date(),
                row['Func_refGene'], row['Gene_refGene'],
